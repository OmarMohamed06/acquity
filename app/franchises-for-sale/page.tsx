...
export default function FranchisesForSalePage() {
  ...
  useEffect(() => {
    ...
    const fetchListings = async () => {
      try {
        console.log("Starting to fetch franchise listings...");
        ...
        // If no results with franchise_sale type, try fetching all and filter by franchise_details existence
        if (!data || data.length === 0) {
          console.warn(
            "No franchise_sale type found, fetching all listings with franchise_details...",
          );
          ...
          // Filter to only listings that have franchise_details
          data =
            allData?.filter(
              (item: any) =>
                item.franchise_details && item.franchise_details.length > 0,
            ) || [];
          console.log("Found listings with franchise_details:", data?.length);
        }

        if (!data || data.length === 0) {
          console.warn("No franchise listings found");
          setListings([]);
          return;
        }
        ...
        const parsed: Listing[] = data.map((listing: any) => {
          ...
          const details =
            listing.franchise_sale_details || listing.franchise_details || null;
          ...
          return {
            ...
            plan: listing.plan,
            price:
              details?.franchise_fee ?? details?.franchiseFee ?? listing.price,
            revenue:
          ...
          >
            <div className="flex flex-col gap-1 md:gap-2">
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm md:text-base">
                Explore franchise opportunities with established brands.
              </p>
            </div>
            ...
            <div className="flex items-center justify-between gap-2 bg-white dark:bg-gray-800 p-3 rounded border border-gray-200 dark:border-gray-800 shadow-sm min-w-0">
              ...
              <div className="flex items-center gap-2 xs:gap-3 shrink-0">
                ...
                >
                  ...
                  <option value="price_low">Low Fee</option>
                  <option value="price_high">High Fee</option>
