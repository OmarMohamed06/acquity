...
import { LAUNCH_MODE, MOCK_FRANCHISE_LISTINGS } from "@/app/constants/launch";
...
// Helper function to convert slug back to original text
const fromSlug = (slug: string, list: string[]): string | undefined => {
  return list.find((item) => toSlug(item) === slug);
};
...
export default function FilteredFranchisesPage() {
  ...
  const parseFilters = () => {
    ...
    if (segments && segments.length > 0) {
      // First segment could be industry or country
      for (const segment of segments) {
        const matchedIndustry = fromSlug(segment, INDUSTRIES);
        const matchedCountry = fromSlug(segment, COUNTRIES);

        if (matchedIndustry) {
          currentIndustry = matchedIndustry;
        }
        if (matchedCountry) {
    ...
    return { currentIndustry, currentCountry };
  };

  const { currentIndustry, currentCountry } = parseFilters();

  // Build filter URL for navigation
  const buildFilterUrl = (industry?: string, country?: string): string => {
    const basePath = "/franchises-for-sale";
    if (industry && country) {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, priceMin, priceMax, segments]);

  useEffect(() => {
    // Skip fetching if in launch mode - use mock data instead
    if (LAUNCH_MODE) {
      setListings(MOCK_FRANCHISE_LISTINGS);
      return;
    }

    const fetchListings = async () => {
      try {
        console.log("Fetching franchise_sale listings from Supabase...");
        ...
        const parsed: Listing[] = data.map((listing: any) => {
          ...
          const details = listing.franchise_sale_details;

          return {
            id: listing.id,
            slug: listing.slug,
            type: listing.type as "franchise_sale",
            category: listing.category || listing.industry,
            country: listing.country || parsedCountry,
            ...
            badge: listing.badge,
            plan: listing.plan,
            price: details?.franchise_fee || listing.price,
            revenue: details?.avg_unit_revenue || listing.revenue,
            profit: details?.avg_unit_profit,
  ...
  const filteredListings = listings.filter((listing) => {
    ...
    const matchesIndustry = !currentIndustry || category === currentIndustry;
  ...
  const getPageTitle = () => {
    if (currentIndustry && currentCountry) {
      return `${currentIndustry} Franchises in ${currentCountry}`;
    }
    if (currentIndustry) {
      return `${currentIndustry} Franchises for Sale`;
    }
    if (currentCountry) {
      return `Franchises for Sale in ${currentCountry}`;
    }
    return "Franchises for Sale";
  };
      ...
      <div className="flex flex-1 min-h-screen relative">
        ...
        >
          <div className="p-5 flex flex-col gap-6">
            ...
            {(currentIndustry || currentCountry) && (
              <div className="flex flex-wrap gap-2">
                {currentIndustry && (
                  <span className="inline-flex items-center gap-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-2 py-1 rounded text-xs font-medium">
                    {currentIndustry}
                    <Link
                      href={buildFilterUrl(undefined, currentCountry)}
                ...
                {currentCountry && (
                  <span className="inline-flex items-center gap-1 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 px-2 py-1 rounded text-xs font-medium">
                    {currentCountry}
                    <Link
                      href={buildFilterUrl(currentIndustry, undefined)}
              ...
              {industryExpanded && (
                <div className="flex flex-col gap-1 pl-1">
                  {INDUSTRIES.map((item) => (
                    <Link
                      key={item}
                      href={buildFilterUrl(item, currentCountry)}
              ...
              {countryExpanded && (
                <div className="flex flex-col gap-1 pl-1 max-h-60 overflow-y-auto">
                  {COUNTRIES.map((item) => (
                    <Link
                      key={item}
                      href={buildFilterUrl(currentIndustry, item)}
                      className={`text-sm py-1 transition-colors ${
                        currentCountry === item
                          ? "text-blue-600 font-semibold"
                          : "text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:underline"
          ...
          >
            <div className="flex flex-col gap-1 md:gap-2">
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm md:text-base">
                Join thriving franchise networks worldwide.
              </p>
            </div>
            ...
            <div className="flex items-center justify-between gap-2 bg-white dark:bg-gray-800 p-3 rounded border border-gray-200 dark:border-gray-800 shadow-sm min-w-0">
              <p className="text-gray-600 dark:text-gray-400 text-xs xs:text-sm font-medium truncate min-w-0 flex-1">
                Showing {sortedListings.length} franchises
              </p>
              <div className="flex items-center gap-2 xs:gap-3 shrink-0">
                ...
                >
                  ...
                  <option value="revenue_high">Revenue</option>
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {paginatedListings.map((listing, index) => (
                <div
                  ...
                  onClick={() =>
                    router.push(`/franchises-for-sale/listing/${listing.slug}`)
                  }
                >
                  {listing.badge && (
                      ...
                      {listing.profit && (
                        <div>
                          ...
                          <p className="font-bold text-green-700 dark:text-green-300 truncate">
                            ${formatNumber(Number(listing.profit))}
                          </p>
                        </div>
                      )}
                    </div>
