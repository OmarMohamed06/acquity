...
export default function InvestmentListingDetailPage() {
  ...
  useEffect(() => {
    const fetchListing = async () => {
      ...
      try {
        ...
        if (data.type === "investment_opportunity") {
          // Fetch investment-specific details (new table)
          ...
          const { data: legacyDetails } = investmentDetails
            ? { data: null }
            : await supabase
          ...
          const details = investmentDetails || legacyDetails || {};

          enrichedData = {
            ...enrichedData,
            capitalRequired:
              details.capital_required ??
              details.minimum_investment ??
              details.min_investment ??
              details.capitalRequired ??
              enrichedData.capitalRequired,
            revenue:
              details.annual_revenue ?? details.revenue ?? enrichedData.revenue,
            profit:
              details.annual_profit ?? details.profit ?? enrichedData.profit,
            equityOffered:
              details.equity_offered_percent ??
              details.equity_offered ??
              details.equity ??
              details.equityOffered ??
              enrichedData.equityOffered,
            impliedValuation:
              details.implied_valuation ??
              details.valuation ??
              details.impliedValuation ??
              enrichedData.impliedValuation,
            companyStage:
              details.company_stage ??
              details.stage ??
              details.companyStage ??
              enrichedData.companyStage,
            scalabilityReason:
              details.scalability_reason ??
              details.scalability ??
              details.scalabilityReason ??
              enrichedData.scalabilityReason,
          };
        }
  ...
  const imageUrl =
    listing.image_url ||
    listing.image ||
    "https://via.placeholder.com/1200x600?text=Investment+Image";
    ...
    <main className="bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500 mb-6 overflow-x-auto">
          ...
          <span className="flex items-center gap-2 whitespace-nowrap">
            <Link
              href={`/investments-for-sale/${toSlug(listing.category)}`}
              className="hover:text-blue-600 cursor-pointer transition-colors"
            >
              {listing.category}
            </Link>
          ...
          <span className="flex items-center gap-2 whitespace-nowrap">
            <Link
              href={`/investments-for-sale/${toSlug(listing.category)}/${toSlug(listing.country)}`}
              className="hover:text-blue-600 cursor-pointer transition-colors"
            >
              {listing.country}
            </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Company Stage & Scalability */}
            {(listing.companyStage || listing.scalabilityReason) && (
              <section className="bg-white border border-gray-200 rounded-lg p-6">
                <h2 className="text-xl font-bold text-gray-900 mb-6">
                  Investment Highlights
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {listing.companyStage && (
                  ...
                  {listing.impliedValuation && (
                    <div className="flex justify-between border-b border-gray-200 pb-2">
                      ...
                      <span className="font-semibold text-gray-900">
                        ${formatNumber(listing.impliedValuation)}
                      </span>
                    </div>
                  )}
                </div>
          ...
          <aside className="lg:col-span-1 space-y-4">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <div className="grid grid-cols-2 gap-4 mb-6">
                {listing.capitalRequired && (
                  <div>
                    <div className="text-xs text-gray-500 uppercase mb-1">
                      Capital Required
                    </div>
                    <div className="text-xl font-bold text-gray-900">
                      ${formatNumber(listing.capitalRequired)}
                    </div>
                  </div>
                )}
              ...
              <div className="flex items-start gap-2 mb-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 leading-relaxed">
                    This listing is restricted to verified users. Create an
                    investor profile to view financials and contact founders.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs leading-relaxed opacity-90">
                    Find promising investment opportunities in the marketplace.
