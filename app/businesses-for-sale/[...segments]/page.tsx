...
export default function FilteredBusinessesPage() {
  ...
  const buildFilterUrl = (industry?: string, country?: string): string => {
    const basePath = "/businesses-for-sale";
    if (industry && country) {
  ...
  useEffect(() => {
    ...
    if (LAUNCH_MODE) {
      setListings(MOCK_BUSINESS_LISTINGS);
      return;
    }

    const fetchListings = async () => {
      try {
        console.log("Fetching business_sale listings from Supabase...");
        ...
        const parsed: Listing[] = data.map((listing: any) => {
          ...
          const details = listing.business_sale_details;

          return {
            id: listing.id,
            slug: listing.slug,
            type: listing.type as "business_sale",
            ...
            badge: listing.badge,
            plan: listing.plan,
            price: details?.asking_price || listing.price,
            revenue: details?.annual_revenue || listing.revenue,
            ebitda: details?.ebitda || listing.ebitda,
          };
        });
  ...
  const getPageTitle = () => {
    if (currentIndustry && currentCountry) {
      return `${currentIndustry} Businesses in ${currentCountry}`;
    }
    if (currentIndustry) {
      return `${currentIndustry} Businesses for Sale`;
    }
    if (currentCountry) {
      return `Businesses for Sale in ${currentCountry}`;
    }
          ...
          >
            ...
            <div className="flex items-center justify-between gap-2 bg-white dark:bg-gray-800 p-3 rounded border border-gray-200 dark:border-gray-800 shadow-sm min-w-0">
              <p className="text-gray-600 dark:text-gray-400 text-xs xs:text-sm font-medium truncate min-w-0 flex-1">
                Showing {sortedListings.length} businesses
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {paginatedListings.map((listing, index) => (
                <div
                  ...
                  onClick={() =>
                    router.push(`/businesses-for-sale/listing/${listing.slug}`)
                  }
                >
                  {listing.badge && (
                      ...
                      {listing.ebitda && (
                        <div>
                          ...
                          <p className="font-bold text-green-700 dark:text-green-300 truncate">
                            ${formatNumber(Number(listing.ebitda))}
                          </p>
                        </div>
                      )}
                    </div>
